...
var mutateTest = function (methodName) {
  it("does not mutate its input", function () {
    ...
    var secondArgLibrary = {
      each: _.identity, map: _.idenftity, reduce: _.identity, reduceRight: _.identity, sortBy: _.identity,
...
//   => [1, 3]
describe("_.map", function (collection, iteratee) {
  mutateTest("map");

  //setup
  //declare a variable to hold the new Array
  var modifiedArray = [];
  //declare the iteratee that will be used in our tests
  var argsToArrayPlus1 = function (value, key, collection) {
    //reuse _.each to push each iteration to the new array.
    _.each(input, function(){
      modifiedArray.push([value + 1, key, collection]);
    });
  }
